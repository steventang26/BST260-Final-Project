...
#using nnet
mynn <- nnet(Deaths ~ ., data = select(train_set),
  size=2, decay=1.0e-5, maxit=50)

preds = predict(mynn, test_set)
RMSE(preds, test_set$Deaths)

R2(preds, test_set$Deaths)
...
#Calculate RMSE 
RMSEsvm=rmse(predYsvm,test_set$Deaths)
RMSEsvm
...
RMSEBst=rmse(PredYBst,test_set$Deaths)
...
RMSE(PredYBst, test_set$Deaths)
modelsvm = svm(Deaths ~ ., train_set)
# not very good performance on the models thru RMSE and Rsquared values
